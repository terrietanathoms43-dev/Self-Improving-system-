...
export async function createOpenAIAdvisory(model:string,input:AssessmentInput,authoritative:AssessmentResult){
  const response=await openAI("/chat/completions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model,temperature:0,messages:[{role:"system",content:"You are a constrained advisory assistant for a Jamaican medical-assistance fund. Do not diagnose. Do not make a final decision. The deterministic score, category, and review requirement are immutable. Explain safeguards and human-review needs using only the supplied de-identified fields. Return JSON only."},{role:"user",content:JSON.stringify({deidentifiedInput:input,lockedScore:authoritative.score,lockedCategory:authoritative.category,lockedSafeguards:{requiresHumanReview:authoritative.requiresHumanReview,noAiOnlyRejection:true}})}],response_format:{type:"json_object"}})}) as {choices?:Array<{message?:{content?:string}}>;usage?:{prompt_tokens?:number;completion_tokens?:number;total_tokens?:number}};
