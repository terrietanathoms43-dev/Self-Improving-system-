...
describe("transparent assessment", () => {
  ...
  it("keeps routine high-confidence cases on the AI-led pathway", () => {
    const result = assessApplication(base);
    expect(determineReviewRoute(result, "routine-case", 0).type).toBe(
      "routine",
    );
  });
  it("mandates full reassessment when a safeguard is present", () => {
    const result = assessApplication({ ...base, missingDocuments: true });
    expect(determineReviewRoute(result, "case-1").type).toBe("mandatory");
  });
  it("supports deterministic quality-assurance sampling", () => {
    const result = assessApplication(base);
    expect(determineReviewRoute(result, "case-2", 100).type).toBe(
      "quality_assurance",
    );
  });
  ...
  it("records accessibility, caregiving, and evidence risks", () => {
    const result = assessApplication({
      ...base,
      disability: true,
      caregiving: true,
    ...
    expect(result.reviewPathway).toContain("Mandatory");
  });
  it("uses the active version thresholds in production categorization", () => {
    const result = assessApplication(base, "rules-2.0", {
      critical: 85,
      high: 65,
      moderate: 45,
    });
    ...
    expect(result.recommendedAction).toContain("routine");
    expect(result.rulesVersion).toBe("rules-2.0");
  });
  it("rejects invalid version threshold ordering", () => {
    expect(() =>
      assessApplication(base, "bad-rules", {
        critical: 60,
        high: 80,
        moderate: 40,
      }),
