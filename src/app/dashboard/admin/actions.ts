...
export async function inviteStaff(fd: FormData) {
  ...
  if (!p.success) throw new Error("Valid staff details are required");
  ...
  const { data: role } = await s
    ...
    .single();
  if (!role) {
    await admin.from("cbg_users").delete().eq("id", data.user.id);
    await admin.auth.admin.deleteUser(data.user.id);
    throw new Error("Role not found");
  }
  const { error: roleError } = await s
    .from("cbg_user_roles")
    .insert({ user_id: data.user.id, role_id: role.id, assigned_by: actor.id });
  if (roleError) {
    await admin.from("cbg_users").delete().eq("id", data.user.id);
    await admin.auth.admin.deleteUser(data.user.id);
    throw roleError;
...
export async function assignRole(fd: FormData) {
  ...
  if (!p.success) throw new Error("Invalid role assignment");
  const s = await createClient();
  const { data: role } = await s
    .from("cbg_roles")
    .select("id")
  ...
  const { error } = await s
    .from("cbg_user_roles")
    .upsert(
      { user_id: p.data.userId, role_id: role.id, assigned_by: actor.id },
...
export async function setStaffActive(fd: FormData) {
  ...
  if (p.userId === actor.id && p.active === "false")
  ...
  await s
    ...
    .insert({
      actor_id: actor.id,
      actor_role: "admin",
      action: p.active === "true" ? "staff_activated" : "staff_deactivated",
      entity_type: "user",
      entity_id: p.userId,
    });
  revalidatePath("/dashboard/admin");
