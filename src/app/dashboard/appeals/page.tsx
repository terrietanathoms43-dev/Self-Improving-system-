...
export default async function AppealsPage({
  ...
  const [{ data: decided }, { data: appeals }] = await Promise.all([
    ...
    s
      .from("cbg_appeals")
      .select(
        "id,reason,status,submitted_at,cbg_applications(reference_number)",
      )
  ...
  const { error: readAuditError } = await s.from("cbg_audit_logs").insert({
    ...
    action: "appeals_workspace_read",
    entity_type: "appeal_register",
    entity_id: "recent",
    metadata: { record_count: appeals?.length ?? 0 },
  });
  if (readAuditError)
    throw new Error("The sensitive read could not be recorded safely");
  const canDecide = actor.roles.some((role) =>
    ["appeals_reviewer", "admin"].includes(role),
  );
  return (
    <>
      <h1 className="text-3xl font-bold">Appeals monitoring</h1>
      <p className="mt-2 text-slate-600">
        Appeals remain separate from the original decision and require an
        independent, explained review.
      </p>
      {success ? (
        <p
          role="status"
      ...
      <Card className="mt-6">
        ...
        <form action={submitAppeal} className="mt-4 grid gap-3">
          ...
          <Textarea
            name="evidence"
            placeholder="Additional evidence received"
          />
          <SubmitButton pendingLabel="Submitting appeal…">
            Submit appeal
          </SubmitButton>
        </form>
      </Card>
      <div className="mt-6 space-y-4">
        {(appeals ?? []).map((appeal) => (
          <Card key={appeal.id}>
            ...
            <h2 className="mt-2 font-mono font-semibold">
              {
                (
                  appeal.cbg_applications as unknown as {
            ...
            {canDecide && appeal.status !== "decided" ? (
              <form action={decideAppeal} className="mt-4 grid gap-3">
                <input type="hidden" name="appealId" value={appeal.id} />
                <Select name="outcome">
                  <option value="upheld">Original decision upheld</option>
                ...
                <Textarea
                  name="explanation"
                  minLength={20}
                  placeholder="Evidence-based explanation"
                  required
                />
                <SubmitButton pendingLabel="Recording decision…">
