...
import { activateVersion, createProposal, decideProposal, refreshPatterns, rollbackVersion, runEvaluation, uploadEvaluationDataset } from "./actions";
export default async function Governance() {
  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2"><Card><h2 className="font-semibold">Authorized evaluation dataset</h2><p className="mt-2 text-sm text-slate-600">JSON only. Case references are hashed before storage.</p><form action={uploadEvaluationDataset} className="mt-4 grid gap-3"><Input name="dataset" type="file" accept="application/json" required/><Button>Upload evaluation dataset</Button></form></Card><Card><h2 className="font-semibold">Run regression evaluation</h2><form action={runEvaluation} className="mt-4 grid gap-3"><Select name="versionId" required><option value="">Draft version</option>{(versions??[]).filter(v=>v.status!=="active").map(v=><option key={v.id} value={v.id}>{v.version}</option>)}</Select><Select name="baselineId" required><option value="">Baseline version</option>{(versions??[]).map(v=><option key={v.id} value={v.id}>{v.version}</option>)}</Select><Input name="datasetName" placeholder="Dataset name" required/><Input name="datasetVersion" placeholder="Dataset version" required/><Button>Run before-and-after evaluation</Button></form></Card></div>
